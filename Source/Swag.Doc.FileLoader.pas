...
begin
  if not Assigned(fSwagDoc) then
    raise Exception.Create('Valid SwagDoc object not assigned');
  ...
  vSwaggerJson := TJSONObject.ParseJSONValue(TEncoding.UTF8.GetString(vSwaggerBytes)) as TJSONObject;

  try
    if not Assigned(vSwaggerJson) then
      raise ESwagErrorLoadSwaggerJsonFile.Create('Stream could not be loaded');

    if Assigned(fSwagDoc.Info) then
